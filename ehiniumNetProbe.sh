...
print_table_terminal() {
	...
	for ((i = 0; i < n; i++)); do
		local t="${TESTS[i]}"
		local r
		r=$(printf "%s" "${RESULTS[i]}" | strip_ansi)
		local d="${DETAILS[i]}"
		((${#t} > w1)) && w1=${#t}
		((${#r} > w2)) && w2=${#r}
		((${#d} > w3)) && w3=${#d}
...
test_icmp_ping() {
	...
	if [[ "$loss_pct" =~ ^[0-9.]+$ ]]; then
		awk -v p="$loss_pct" 'BEGIN{exit !(p==0)}' && {
			add_row "icmp_ping" "$(ok)" "loss ${loss_pct}%, rtt ${rtt} ms"
			return
		}
		awk -v p="$loss_pct" 'BEGIN{exit !(p<=20)}' && {
			add_row "icmp_ping" "$(warn)" "loss ${loss_pct}%, rtt ${rtt} ms"
			return
		}
...
wizard_loop() {
	...
	while true; do
		...
		if [[ "$SUITE" == "custom" ]]; then
			prompt_line_default "TCP echo port [$TCP_ECHO_PORT]: " "$TCP_ECHO_PORT"
			TCP_ECHO_PORT="$PROMPT_VALUE"
			prompt_line_default "UDP echo port [$UDP_ECHO_PORT]: " "$UDP_ECHO_PORT"
			UDP_ECHO_PORT="$PROMPT_VALUE"
			prompt_line_default "TLS port [$TLS_PORT]: " "$TLS_PORT"
			TLS_PORT="$PROMPT_VALUE"
			prompt_line_default "iperf3 port base [$IPERF_PORT_BASE]: " "$IPERF_PORT_BASE"
			IPERF_PORT_BASE="$PROMPT_VALUE"
			prompt_line_default "iperf3 port count [$IPERF_PORT_COUNT]: " "$IPERF_PORT_COUNT"
			IPERF_PORT_COUNT="$PROMPT_VALUE"
			prompt_line_default "iperf3 explicit ports CSV [$IPERF_PORTS_CSV]: " "$IPERF_PORTS_CSV"
			IPERF_PORTS_CSV="$PROMPT_VALUE"
			prompt_line_default "iperf3 TCP duration seconds [$IPERF_TCP_TIME]: " "$IPERF_TCP_TIME"
			IPERF_TCP_TIME="$PROMPT_VALUE"
			prompt_line_default "iperf3 TCP parallel [$IPERF_TCP_PARALLEL]: " "$IPERF_TCP_PARALLEL"
			IPERF_TCP_PARALLEL="$PROMPT_VALUE"
			prompt_line_default "iperf3 TCP fallback parallel [$IPERF_TCP_FALLBACK_PARALLEL]: " "$IPERF_TCP_FALLBACK_PARALLEL"
			IPERF_TCP_FALLBACK_PARALLEL="$PROMPT_VALUE"
			prompt_line_default "iperf3 UDP duration seconds [$IPERF_UDP_TIME]: " "$IPERF_UDP_TIME"
			IPERF_UDP_TIME="$PROMPT_VALUE"
			prompt_line_default "iperf3 UDP bandwidth [$IPERF_UDP_BW]: " "$IPERF_UDP_BW"
			IPERF_UDP_BW="$PROMPT_VALUE"
			prompt_line_default "iperf3 UDP fallback bandwidth [$IPERF_UDP_FALLBACK_BW]: " "$IPERF_UDP_FALLBACK_BW"
			IPERF_UDP_FALLBACK_BW="$PROMPT_VALUE"
			prompt_line_default "iperf3 server timeout seconds [$SERVER_IPERF_TIMEOUT]: " "$SERVER_IPERF_TIMEOUT"
			SERVER_IPERF_TIMEOUT="$PROMPT_VALUE"
			prompt_line_default "iperf3 server kill timeout seconds [$SERVER_IPERF_KILL_TIMEOUT]: " "$SERVER_IPERF_KILL_TIMEOUT"
			SERVER_IPERF_KILL_TIMEOUT="$PROMPT_VALUE"
			prompt_line_default "iperf3 rcv-timeout milliseconds [$IPERF_RCV_TIMEOUT_MS]: " "$IPERF_RCV_TIMEOUT_MS"
			IPERF_RCV_TIMEOUT_MS="$PROMPT_VALUE"
			prompt_line_default "Soak loops [$SOAK_LOOPS]: " "$SOAK_LOOPS"
			SOAK_LOOPS="$PROMPT_VALUE"
			prompt_line_default "Soak interval seconds [$SOAK_INTERVAL]: " "$SOAK_INTERVAL"
			SOAK_INTERVAL="$PROMPT_VALUE"
...
parse_args() {
	...
	if [[ "$MODE" != "client" && "$MODE" != "server" ]]; then
		usage
		err "First arg must be client or server (or run interactive)."
		exit 1
	fi
	...
	while [[ $# -gt 0 ]]; do
		...
		--iperf-ports)
			...
			;;
		--iperf-tcp-parallel)
			IPERF_TCP_PARALLEL="${2:-}"
			shift 2
			;;
		--iperf-tcp-fallback-parallel)
			IPERF_TCP_FALLBACK_PARALLEL="${2:-}"
			shift 2
			;;
		--iperf-tcp-time)
			IPERF_TCP_TIME="${2:-}"
		...
		--iperf-udp-bw)
			...
			shift 2
			;;
		--iperf-udp-fallback-bw)
			IPERF_UDP_FALLBACK_BW="${2:-}"
			shift 2
			;;
		--iperf-udp-time)
			IPERF_UDP_TIME="${2:-}"
			shift 2
			;;
		--iperf-server-timeout)
			SERVER_IPERF_TIMEOUT="${2:-}"
			shift 2
			;;
		--iperf-server-kill-timeout)
			SERVER_IPERF_KILL_TIMEOUT="${2:-}"
			shift 2
			;;
		--iperf-rcv-timeout-ms)
			IPERF_RCV_TIMEOUT_MS="${2:-}"
			shift 2
			;;
		--format)
			OUTPUT_FORMAT="${2:-terminal}"
			shift 2
			;;
		--tsv)
			TSV_OUT="${2:-}"
